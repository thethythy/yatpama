 ...
 * Return value: 0 if OK
 */
int generate_key(T_Shared * pt_sh, char * argv0, uint8_t * key, uint8_t * msecret) {
    int error = 0; 
 ...
 * Return value: 0 if OK
 */
int do_command_key(T_Shared * pt_sh, char * exec_name, uint8_t * key, DLList * list) {
    char passwd[PWD_MAX_SIZE];

    get_shared_cmd_1arg(pt_sh, passwd, PWD_MAX_SIZE); // Get the password
    int error = generate_key(pt_sh, exec_name, key, (uint8_t *)passwd); // Generate the key

    if (!error) {
        memset(passwd, 0, PWD_MAX_SIZE);
        
        *list = load_data(pt_sh, key, FILE_DATA_NAME); // Load and control data
...
void * thread_core(void * t_arg) {
    ...
    uint8_t key[AES_KEYLEN]; // Encryption key
    ...
    while(loop_again) {
        ...
        switch (core_cmd) {
            ...
            case CORE_CMD_DEL_P1:
                if (has_key)
                    nbEntry = do_command_delete_get_entry(pt_sh, key, list); // We store the entry number to be deleted
                else
                    add_shared_cmd_1arg(pt_sh, HMI_CMD_ALERT, "...but we don't have password!");
    ...
    memset(key, 0, AES_KEYLEN); // We forget the master key
